...
export const ArticleCard = ({ article }:any) => {
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-2xl shadow-md hover:shadow-xl transition-all border border-purple-50 overflow-hidden"
    >
      {/* Article Image */}
      {article.image && (
      ...
      <div className="p-4 sm:p-6">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-100">
          <div className="flex items-center space-x-4 sm:space-x-6">
            ...
            >
              <Heart className={`w-4 h-4 sm:w-5 sm:h-5 ${isLiked ? 'fill-purple-600 text-purple-600' : ''}`} />
              <span className="text-xs sm:text-sm font-medium">{article.likes}</span>
            </button>
            
            <button className="flex items-center space-x-1 sm:space-x-2 text-gray-600 hover:text-purple-600 transition-colors">
          ...
          >
            <Bookmark className={`w-4 h-4 sm:w-5 sm:h-5 ${isBookmarked ? 'fill-purple-600 text-purple-600' : ''}`} />
          </button>
        </div>
      </div>
    </motion.div>
