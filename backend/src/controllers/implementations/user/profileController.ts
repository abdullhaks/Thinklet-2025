...
export default class ProfileController implements IProfileController {
  ...
  async updateProfileImageController(req: any, res: any): Promise<void> {
    try {
      if (
        ...
        return res
          .status(400)
          .json({ message: "No profile image uploaded", code: "NO_FILE" });
      }
      const profileImage = (req.files as MulterFiles)?.profile?.[0];
      ...
      const response = await this._profileService.updateProfileImageService(
        userId,
        profile
      );
  ...
  async updateProfileController(req: any, res: any): Promise<void> {
    try {
      const { userId, firstName, lastName, email, phone, preferences } =
        req.body;

      if (!userId) {
        return res
          .status(400)
          .json({ message: "User ID is required", code: "NO_USER_ID" });
      }
      ...
      const response = await this._profileService.updateProfileService(
        profileData
      );
      return res
        .status(200)
